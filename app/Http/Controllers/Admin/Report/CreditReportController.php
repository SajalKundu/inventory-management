...
class CreditReportController extends Controller
{
    public function index(Request $request)
    {
        if($request->start_date != null && $request->end_date != null){
            ...
            $creditors = Creditors::where('created_at', '>=', Carbon::parse($start_date)->format('Y-m-d'))
    ...
    public function creditReportDataDownload(Request $request)
    {
        $start_date = Carbon::parse($request->start_date)->format('Y-m-d');
        $end_date = Carbon::parse($request->end_date)->format('Y-m-d');
        $creditors = Creditors::whereDate('created_at', '>=', Carbon::parse($start_date)->format('Y-m-d'))
